...
export const EndScene: React.FC = () => {
	...
	const opacity = currentFrame <= fps ? 0 : interpolate(
		currentFrame,
		[fps, 1.5 * fps],
		[0, 1],
		{
	...
	const offsetTop = `${interpolate(
		...
		[-920, -70],
		{
			easing: Easing.bezier(.24,.92,.63,1.09),
			extrapolateRight: 'clamp',
			extrapolateLeft: 'clamp',
		}
	)}px`;
		...
		<AbsoluteFill className="bg-primary">
			<div className="w-full h-full flex flex-row">
				<div className="flex-1 relative">
					<div className="end-card bg-white absolute rounded-b-6xl z-10 px-20 pb-20 pt-36" style={{marginTop: offsetTop}}>
					...
					<div className="end-card-shadow bg-whiteShadow absolute top-0 right-0 rounded-b-6xl" style={{marginTop: offsetTop}}/>
				</div>
				<div className="flex-1 flex flex-col justify-center items-center gap-12" style={{opacity}}>
